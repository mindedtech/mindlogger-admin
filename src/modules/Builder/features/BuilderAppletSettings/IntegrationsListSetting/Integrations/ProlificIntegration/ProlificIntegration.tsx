...
const ProlifcIntegrationApplet = ({ appletData }: ProlificIntegrationAppletProps) => {
  ...
  useEffect(() => {
    const checkProlificApiToken = async () => {
      const foundProlificIntegration =
        appletData.integrations?.some((i) => i.integrationType === IntegrationTypes.Prolific) ??
        false;
      const hasProlificIntegration =
        foundProlificIntegration || (await prolificIntegrationExists(appletData.id));

      setState((prevState) => ({
        ...prevState,
        apiTokenExists: hasProlificIntegration,
      }));
    };
